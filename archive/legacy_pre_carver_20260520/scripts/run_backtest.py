...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--trend-min-stop-atr-multiple",
        type=float,
        help="Experiment only: require trend initial stop to be at least this many ATRs from entry",
    )
    parser.add_argument(
        "--trend-max-entry-adverse-deviation-r",
        type=float,
        help="Experiment only: skip trend entry if actual trigger has moved farther than this many initial R from plan",
    )
    parser.add_argument(
        "--trend-min-first-target-rr",
        type=float,
        help="Experiment only: require trend first target reward/risk to reach this threshold",
    )
    parser.add_argument(
        "--trend-entry-confirmation-bars",
        type=int,
        help="Experiment only: require this many trend entry confirmation bars before opening",
    )
    parser.add_argument(
        "--trend-initial-stop-grace-bars",
        type=int,
        help="Experiment only: ignore trend initial stop touches for this many bars after entry",
    )
    return parser
...
def _print_trade_debug(*, trades, prefix: str = "") -> None:
    for idx, trade in enumerate(trades, start=1):
        ...
        print(f"{label}_planned_entry_ref={trade.meta.get('planned_entry_ref', '')}")
        print(f"{label}_initial_stop_price={trade.meta.get('initial_stop_price', '')}")
        print(f"{label}_initial_tp1_price={trade.meta.get('initial_tp1_price', '')}")
        print(f"{label}_initial_tp2_price={trade.meta.get('initial_tp2_price', '')}")
        print(f"{label}_entry_trigger_deviation={trade.meta.get('entry_trigger_deviation', '')}")
        print(f"{label}_entry_trigger_deviation_pct={trade.meta.get('entry_trigger_deviation_pct', '')}")
        print(f"{label}_entry_rr={trade.meta.get('entry_rr', '')}")
        print(f"{label}_entry_admission_rr={trade.meta.get('entry_admission_rr', '')}")
...
def _config_with_trend_experiment_overrides(config: dict, args) -> dict:
    out = dict(config)
    pre_market_cfg = dict(out.get("pre_market") or {})
    if args.trend_min_stop_atr_multiple is not None:
        pre_market_cfg["trend_min_stop_atr_multiple"] = float(args.trend_min_stop_atr_multiple)
    if args.trend_max_entry_adverse_deviation_r is not None:
        pre_market_cfg["trend_max_entry_adverse_deviation_r"] = float(args.trend_max_entry_adverse_deviation_r)
    if args.trend_min_first_target_rr is not None:
        pre_market_cfg["trend_min_first_target_rr"] = float(args.trend_min_first_target_rr)
    if args.trend_entry_confirmation_bars is not None:
        pre_market_cfg["trend_entry_confirmation_bars"] = int(args.trend_entry_confirmation_bars)
    if args.trend_initial_stop_grace_bars is not None:
        pre_market_cfg["trend_initial_stop_grace_bars"] = int(args.trend_initial_stop_grace_bars)
    out["pre_market"] = pre_market_cfg
    return out
