...
def _trend_story_fields(df: pd.DataFrame, *, direction: str) -> dict[str, Any]:
    quality = trend_opportunity_quality_from_daily(df, direction=direction)
    return {
        "trend_story_quality_score": float(quality.get("trend_opportunity_quality_score") or 0.0),
        "trend_story_quality_status": str(quality.get("trend_opportunity_quality_status") or ""),
        "trend_story_bucket": str(quality.get("trend_opportunity_bucket") or ""),
        "trend_story_budget_margin_pct": float(quality.get("trend_budget_margin_pct") or 0.03),
        "trend_direction_stability_score": float(quality.get("trend_direction_stability_score") or 0.0),
        "trend_structure_health_score": float(quality.get("trend_structure_health_score") or 0.0),
        "trend_structure_bucket": str(quality.get("trend_structure_bucket") or ""),
        "trend_remaining_space_score": float(quality.get("trend_remaining_space_score") or 0.0),
        "trend_volatility_fit_score": float(quality.get("trend_volatility_fit_score") or 0.0),
        "trend_participation_score": float(quality.get("trend_participation_score") or 0.0),
    }
...
def _build_candidate(
    *,
    ...
    trend_story_fields = _trend_story_fields(df, direction=trend_direction)
    ...
    candidate = {
        ...
        "entry_pool_reason": "",
        "phase1_story_budget_margin_pct": trend_story_fields["trend_story_budget_margin_pct"],
    ...
    candidate.update(trend_story_fields)
