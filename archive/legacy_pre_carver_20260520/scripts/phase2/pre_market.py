...
def _build_trend_candidate_plan(
    *,
    ...
    entry = last
    min_stop_atr_multiple = max(float(cfg.get("trend_min_stop_atr_multiple", 0.0)), 0.0)
    min_first_target_rr = max(float(cfg.get("trend_min_first_target_rr", 0.0)), 0.0)
    second_target_rr_relax_threshold = max(float(cfg.get("trend_second_target_rr_relax_threshold", 0.0)), 0.0)
    relaxed_first_target_rr = max(float(cfg.get("trend_relaxed_first_target_rr", min_first_target_rr)), 0.0)
    ...
    if direction == "long":
        ...
        if signal_type == "TrendBreak":
            ...
            stop = anchor - 0.5 * atr
        if atr > 0 and min_stop_atr_multiple > 0:
            stop = min(stop, entry - atr * min_stop_atr_multiple)
        ...
        if signal_type == "TrendBreak":
            ...
            stop = anchor + 0.5 * atr
        if atr > 0 and min_stop_atr_multiple > 0:
            stop = max(stop, entry + atr * min_stop_atr_multiple)
    ...
    extended_target = rr < 1.0 and admission_rr >= 1.0
    effective_min_first_target_rr = min_first_target_rr
    if second_target_rr_relax_threshold > 0 and admission_rr >= second_target_rr_relax_threshold:
        effective_min_first_target_rr = min(min_first_target_rr, relaxed_first_target_rr)
    first_target_rr_ok = rr >= effective_min_first_target_rr if effective_min_first_target_rr > 0 else True
    first_target_gate_threshold = effective_min_first_target_rr if effective_min_first_target_rr > 0 else 1.0
    rr_ok = admission_rr >= 1.0 and first_target_rr_ok
    ...
    return {
        ...
        "phase2_first_target_rr_gate_passed": bool(rr >= first_target_gate_threshold),
        ...
        "entry_plan_type": entry_plan_type,
        "management_note": management_note,
        "trend_stop_min_atr_multiple": float(min_stop_atr_multiple),
        "trend_min_first_target_rr": float(min_first_target_rr),
        "trend_second_target_rr_relax_threshold": float(second_target_rr_relax_threshold),
        "trend_relaxed_first_target_rr": float(relaxed_first_target_rr),
        "trend_effective_min_first_target_rr": float(effective_min_first_target_rr),
        "downgrade_reason": downgrade_reason,
...
def _build_plan_payload(
    *,
    ...
    if candidate:
        ...
        trend_stop_min_atr_multiple = float(candidate.get("trend_stop_min_atr_multiple", 0.0))
        trend_min_first_target_rr = float(candidate.get("trend_min_first_target_rr", 0.0))
        trend_second_target_rr_relax_threshold = float(candidate.get("trend_second_target_rr_relax_threshold", 0.0))
        trend_relaxed_first_target_rr = float(candidate.get("trend_relaxed_first_target_rr", trend_min_first_target_rr))
        trend_effective_min_first_target_rr = float(
            candidate.get("trend_effective_min_first_target_rr", trend_min_first_target_rr)
        )
        ...
        entry_plan_type = ""
        management_note = ""
        trend_stop_min_atr_multiple = 0.0
        trend_min_first_target_rr = 0.0
        trend_second_target_rr_relax_threshold = 0.0
        trend_relaxed_first_target_rr = 0.0
        trend_effective_min_first_target_rr = 0.0
        downgrade_reason = ""
        signal_ref_level = 0.0
    ...
    return {
        ...
        "entry_plan_type": entry_plan_type,
        "management_note": management_note,
        "trend_stop_min_atr_multiple": trend_stop_min_atr_multiple,
        "trend_min_first_target_rr": trend_min_first_target_rr,
        "trend_second_target_rr_relax_threshold": trend_second_target_rr_relax_threshold,
        "trend_relaxed_first_target_rr": trend_relaxed_first_target_rr,
        "trend_effective_min_first_target_rr": trend_effective_min_first_target_rr,
        "downgrade_reason": downgrade_reason,
