...
def _phase2_debug_snapshot_from_raw(raw: dict) -> dict[str, object]:
    ...
    admission_rr = float(raw.get("admission_rr") or rr)
    return {
        ...
        "phase2_risk_gate_passed": bool(raw.get("phase2_risk_gate_passed"))
        if "phase2_risk_gate_passed" in raw
        else False,
...
def _phase2_debug_snapshot_from_plan(plan: TradePlan | None) -> dict[str, object]:
    ...
    return {
        ...
        "admission_rr": float(plan.meta.get("admission_rr") or _rr_from_plan(plan)),
        "risk_pct": float(plan.meta.get("risk_pct") or 0.0),
        "phase2_risk_gate_passed": True,
        "scores": dict(plan.meta.get("scores") or {}),
...
def _entry_adverse_deviation_r(plan: TradePlan, entry_price: float) -> float:
    risk_distance, _, _ = _execution_plan_distances(plan)
    if risk_distance <= 0:
        return 0.0
    if plan.direction == "long":
        adverse_deviation = entry_price - plan.entry_ref
    else:
        adverse_deviation = plan.entry_ref - entry_price
    return max(adverse_deviation, 0.0) / risk_distance


def _entry_trigger_allowed(plan: TradePlan, entry_price: float, pre_market_cfg: dict) -> bool:
    if _entry_family_for_plan(plan) != "trend":
        return True
    try:
        max_adverse_r = float(pre_market_cfg.get("trend_max_entry_adverse_deviation_r", 0.0))
    except (TypeError, ValueError):
        max_adverse_r = 1.0
    if max_adverse_r <= 0:
        return True
    return _entry_adverse_deviation_r(plan, entry_price) <= max_adverse_r


def _int_cfg(pre_market_cfg: dict, key: str, default: int) -> int:
    try:
        return int(pre_market_cfg.get(key, default))
    except (TypeError, ValueError):
        return default


def _trend_entry_confirmation_bars(plan: TradePlan, pre_market_cfg: dict) -> int:
    if _entry_family_for_plan(plan) != "trend":
        return 1
    return max(_int_cfg(pre_market_cfg, "trend_entry_confirmation_bars", 1), 1)


def _trend_initial_stop_grace_bars(plan: TradePlan, pre_market_cfg: dict) -> int:
    if _entry_family_for_plan(plan) != "trend":
        return 0
    return max(_int_cfg(pre_market_cfg, "trend_initial_stop_grace_bars", 0), 0)
    ...
    meta.update(
        {
            ...
            "initial_tp1_price": float(tp1),
            "initial_tp2_price": float(tp2),
            "entry_trigger_deviation": float(entry_price - plan.entry_ref),
            "entry_trigger_deviation_pct": float((entry_price - plan.entry_ref) / plan.entry_ref)
            if plan.entry_ref > 0
            else 0.0,
            "entry_rr": float(_reward_risk(plan.direction, entry=entry_price, target=tp1, stop=stop)),
            "entry_admission_rr": float(_reward_risk(plan.direction, entry=entry_price, target=tp2, stop=stop)),
...
def run_case_from_frames(
    *,
    ...
    diagnostics: dict[str, int] = {
        ...
        "phase3_entry_adverse_deviation_rejects": 0,
        "phase3_entry_confirmation_wait_bars": 0,
        "phase3_initial_stop_grace_skips": 0,
        "trades_opened": 0,
    ...
    tp1_exit_time: pd.Timestamp | None = None
        ...
        for minute_idx in range(len(day_minutes)):
            ...
            if active_plan is None:
                ...
                if any(
                    ...
                    if entry_confirmation_trade_id != day_plan.trade_id:
                        entry_confirmation_trade_id = day_plan.trade_id
                        entry_confirmation_count = 0
                    entry_confirmation_count += 1
                    if entry_confirmation_count < _trend_entry_confirmation_bars(day_plan, pre_market_cfg):
                        diagnostics["phase3_entry_confirmation_wait_bars"] += 1
                        continue
                    entry_time = pd.Timestamp(current_minute["datetime"])
                    entry_price = float(current_minute["close"])
                    if not _entry_trigger_allowed(day_plan, entry_price, pre_market_cfg):
                        diagnostics["phase3_entry_adverse_deviation_rejects"] += 1
                        continue
            ...
            if _stop_touched(case.direction, current_minute, stop_price):
                if (
                    not tp1_hit
                    and bars_held <= _trend_initial_stop_grace_bars(active_plan, pre_market_cfg)
                ):
                    diagnostics["phase3_initial_stop_grace_skips"] += 1
                    exit = None
                else:
                ...
                if not tp1_hit and _tp1_touched(case.direction, current_minute, active_plan):
                    ...
                    tp1_exit_time = pd.Timestamp(current_minute["datetime"])
