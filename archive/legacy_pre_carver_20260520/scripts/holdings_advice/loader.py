        ...
        contexts.append(
            {
                ...
                "recommendation": None
                if _is_unlinked_holding_note(note)
                else recommendation_by_id.get(record_id),
            }
        )
    return contexts
...
def _autofill_recommendation_sheet(path: Path, *, reports_dir: Path) -> None:
    ...
    for row_idx in range(2, hold_sheet.max_row + 1):
        ...
        if not symbol or direction not in {"long", "short"}:
            continue
        if _is_unlinked_holding_note(_cell_value(hold_sheet, row_idx, hold_headers, "备注")):
            continue
...
def _is_unlinked_holding_note(value: object) -> bool:
    text = str(value or "").strip()
    return any(marker in text for marker in ("不关联系统推荐", "不关联推荐", "主观持仓"))
