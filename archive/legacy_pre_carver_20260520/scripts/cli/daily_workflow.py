...
from shared.position_sizing import calculate_position_sizing, positive_int_floor
...
def _merge_strategy_result(
    *,
    ...
    for key in (
        ...
        "trend_story_quality_score",
        "trend_story_quality_status",
        "trend_story_bucket",
        "trend_story_budget_margin_pct",
        "trend_structure_bucket",
        "trend_structure_health_score",
        "trend_direction_stability_score",
        "trend_remaining_space_score",
        "trend_volatility_fit_score",
        "trend_participation_score",
    ):
        if key in cand:
...
def _positive_int_floor(value: float) -> int:
    return positive_int_floor(value)
...
def _position_budget_source(config: dict) -> str:
    pre_cfg = config.get("pre_market") or {}
    source = str(pre_cfg.get("position_budget_source") or "phase2_score").strip().lower()
    if source in {"phase1_story", "story", "phase1"}:
        return "phase1_story"
    return "phase2_score"


def _phase1_story_budget_margin_pct(row: dict) -> float:
    value = _as_float(
        row.get("phase1_story_budget_margin_pct")
        or row.get("trend_story_budget_margin_pct")
        or row.get("story_budget_margin_pct"),
        0.0,
    )
    if value <= 0:
        value = 0.03
...
def _apply_portfolio_position_sizing(rows: list[dict], config: dict) -> None:
    ...
    if budget_source == "phase1_story":
        weights = [_phase1_story_budget_margin_pct(row) for row in rows]
        requested_budgets = [account_equity * weight for weight in weights]
        requested_total = sum(requested_budgets)
        budget_scale = min(1.0, margin_pool / requested_total) if requested_total > 0 else 0.0
    ...
    for idx, (row, weight) in enumerate(zip(rows, weights, strict=False)):
        ...
        if budget_source == "phase1_story":
            score_margin_budget = requested_budgets[idx] * budget_scale if idx < len(requested_budgets) else 0.0
        else:
        ...
        precomputed_risk_lots = int(row.get("max_lots_by_risk") or row.get("risk_lots") or 0) or None
        sizing = calculate_position_sizing(
            score_margin_budget=score_margin_budget,
            portfolio_margin_budget=margin_pool,
            risk_budget=risk_budget,
            margin_per_lot=margin_per_lot,
            risk_per_lot=risk_per_lot,
            risk_lots=precomputed_risk_lots,
        )

        row["position_budget_source"] = budget_source
        row["position_weight"] = float(weight)
        row["position_weight_pct"] = (
            float(score_margin_budget / margin_pool)
            if budget_source == "phase1_story" and margin_pool > 0
            else float(weight / total_weight) if total_weight > 0 else 0.0
        )
        ...
        row["score_lots"] = int(sizing.score_lots)
        row["portfolio_lots"] = int(sizing.portfolio_lots)
        row["risk_lots"] = int(sizing.risk_lots)
        row["suggested_lots"] = int(sizing.suggested_lots)
        row["suggested_margin"] = float(sizing.suggested_margin)
        row["suggested_stop_risk"] = float(sizing.suggested_stop_risk)
        row["sizing_limited_by"] = sizing.sizing_limited_by
        row["sizing_zero_lot_reason"] = sizing.sizing_zero_lot_reason
        row["phase2_position_size_passed"] = bool(sizing.suggested_lots >= 1)
        if budget_source == "phase1_story":
            log.info(
                "     %s (%s) Phase1故事预算%.1f%% 预算%.0f 每手保证金%.0f 每手止损%.0f "
                ...
                risk_per_lot,
                sizing.score_lots,
                sizing.portfolio_lots,
                sizing.risk_lots,
                sizing.suggested_lots,
            )
        else:
            log.info(
...
def save_targets(
    ...
    def _format_risk_control_note(t: dict) -> str:
        ...
        if score_margin_budget > 0 or score_lots > 0 or portfolio_lots > 0 or risk_lots > 0:
            budget_label = "Phase1故事预算" if t.get("position_budget_source") == "phase1_story" else "Phase2权重预算"
            note = (
                f"{budget_label}{score_margin_budget:.0f}元，"
    ...
    lines.append("")
    lines.append("适用于趋势已确立（Wyckoff=markup/markdown）且Phase 2趋势方向明确时：")
    lines.append("")
