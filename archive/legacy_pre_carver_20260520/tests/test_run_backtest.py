...
def test_main_passes_trend_experiment_overrides(monkeypatch):
    from backtest.cases import get_case
    ...
    monkeypatch.setattr(
        run_backtest,
        "load_config",
        lambda: {"pre_market": {"base": True}, "intraday": {}, "tqsdk": {"account": "a", "password": "b"}},
    )
    monkeypatch.setattr(
        run_backtest,
        "load_case_frames_with_tqbacktest",
    ...
    def fake_run_case_from_frames(**kwargs):
        seen["pre_market_cfg"] = dict(kwargs["pre_market_cfg"])
        return BacktestResult(case_id="lh0_short", trades=[], summary={}, diagnostics={})

    monkeypatch.setattr(run_backtest, "run_case_from_frames", fake_run_case_from_frames)
    monkeypatch.setattr(
        run_backtest,
        "summarize_trades",
        lambda trades: {"num_trades": len(trades), "wins": 0, "win_rate": 0.0},
    )
    ...
    assert exit_code == 0
    assert seen["pre_market_cfg"]["trend_min_stop_atr_multiple"] == 1.5
    assert seen["pre_market_cfg"]["trend_max_entry_adverse_deviation_r"] == 1.0
    assert seen["pre_market_cfg"]["trend_min_first_target_rr"] == 1.2
    assert seen["pre_market_cfg"]["trend_entry_confirmation_bars"] == 2
    assert seen["pre_market_cfg"]["trend_initial_stop_grace_bars"] == 3
...
def test_main_prints_single_case_trade_debug(monkeypatch, capsys):
    ...
    trade = TradeRecord(
        ...
        meta={
            ...
            "planned_entry_ref": 13980.0,
            "initial_stop_price": 14100.0,
            "initial_tp1_price": 13880.0,
            "initial_tp2_price": 13740.0,
            "entry_trigger_deviation": 20.0,
            "entry_trigger_deviation_pct": 20.0 / 13980.0,
            "entry_rr": 1.0,
            "entry_admission_rr": 2.4,
        },
    )
    ...
    assert "debug_trade_1_planned_entry_ref=13980.0" in captured
    assert "debug_trade_1_initial_stop_price=14100.0" in captured
    assert "debug_trade_1_initial_tp1_price=13880.0" in captured
    assert "debug_trade_1_initial_tp2_price=13740.0" in captured
    assert "debug_trade_1_entry_trigger_deviation=20.0" in captured
    assert "debug_trade_1_entry_rr=1.0" in captured
    assert "debug_trade_1_entry_admission_rr=2.4" in captured
...
def test_main_applies_trend_backtest_parameter_overrides(monkeypatch):
    case = BacktestCase(
        case_id="lh0_trend_long",
        symbol="LH0",
        name="生猪",
        direction="long",
        start_dt=date(2025, 1, 1),
    ...
    monkeypatch.setattr(
        run_backtest,
        "load_config",
        lambda: {"pre_market": {"existing": "keep"}, "intraday": {}, "tqsdk": {"account": "a", "password": "b"}},
    )
    monkeypatch.setattr(
        run_backtest,
        "load_case_frames_with_tqbacktest",
    ...
    def fake_run_case_from_frames(**kwargs):
        seen["pre_market_cfg"] = kwargs["pre_market_cfg"]
        return BacktestResult(case_id="lh0_trend_long", trades=[], summary={"num_trades": 0}, diagnostics={})
    ...
    assert exit_code == 0
    assert seen["pre_market_cfg"]["existing"] == "keep"
    assert seen["pre_market_cfg"]["trend_min_stop_atr_multiple"] == pytest.approx(1.5)
    assert seen["pre_market_cfg"]["trend_max_entry_adverse_deviation_r"] == pytest.approx(1.0)
