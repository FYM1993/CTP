...
def test_build_trend_universe_uses_price_trend_without_phase1(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda df, direction: {
            "trend_opportunity_quality_score": 72.0 if direction == "long" else 54.0,
            "trend_opportunity_quality_status": "candidate_v1",
            "trend_opportunity_bucket": "high" if direction == "long" else "medium",
            "trend_budget_margin_pct": 0.30 if direction == "long" else 0.15,
            "trend_structure_bucket": "healthy_continuation",
            "trend_structure_health_score": 80.0,
            "trend_direction_stability_score": 75.0,
            "trend_remaining_space_score": 65.0,
            "trend_volatility_fit_score": 70.0,
            "trend_participation_score": 60.0,
        },
        raising=False,
    )
    ...
    assert selected[0]["trend_story_bucket"] == "high"
    assert selected[0]["phase1_story_budget_margin_pct"] == 0.30
    assert selected[1]["trend_story_bucket"] == "medium"
    assert selected[1]["phase1_story_budget_margin_pct"] == 0.15
