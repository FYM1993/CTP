...
def test_load_holding_contexts_keeps_subjective_holding_unlinked(tmp_path: Path) -> None:
    workbook = Workbook()
    holdings_sheet = workbook.active
    holdings_sheet.title = "持仓录入"
    holdings_sheet.append(["记录ID", "合约代码", "品种名称", "方向", "手数", "开仓均价", "开仓日期", "推荐日期", "备注"])
    holdings_sheet.append(
        [
            ...
            "2026-04-30",
            "",
            "2026-04-30手动录入：主观买入1手，开仓均价11295；不关联系统推荐",
        ]
    )
    ...
    path = tmp_path / "holdings-2026-04-30.xlsx"
    workbook.save(path)

    reports_dir = tmp_path / "reports"
    reports_dir.mkdir()
    (reports_dir / "2026-04-30_targets.json").write_text(
        json.dumps(
            {
                "date": "2026-04-30",
                "targets": [
