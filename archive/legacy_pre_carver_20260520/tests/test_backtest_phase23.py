...
def test_run_case_from_frames_opens_long_and_exits_on_tp2_intrabar_touch(monkeypatch):
    ...
    assert trade.meta["tp1_exit_time"] == "2025-01-06 09:33:00"
...
def test_run_case_from_frames_can_require_multiple_trend_confirmation_bars(monkeypatch):
    minute_df = _build_minute_df(
        [
            ("2025-01-06 09:31:00", 100.0, 101.0, 99.0, 100.0),
            ("2025-01-06 09:32:00", 100.0, 102.0, 99.0, 101.0),
            ("2025-01-06 09:33:00", 101.0, 104.0, 100.0, 103.0),
        ]
    )
    ...
    result = run_case_from_frames(
        ...
        minute_df=minute_df,
        plan_factory=lambda **_kwargs: _trend_long_plan(),
        pre_market_cfg={"trend_entry_confirmation_bars": 2},
        signal_cfg={},
    )

    assert len(result.trades) == 1
    assert result.trades[0].entry_time == "2025-01-06 09:32:00"
    assert result.trades[0].exit_reason == "end_of_data"
    assert result.diagnostics["phase3_entry_confirmation_wait_bars"] == 1


def test_run_case_from_frames_can_apply_initial_stop_grace_for_trend_entries(monkeypatch):
    minute_df = _build_minute_df(
        [
            ("2025-01-06 09:31:00", 100.0, 101.0, 99.0, 100.0),
            ("2025-01-06 09:32:00", 100.0, 100.5, 95.0, 97.0),
            ("2025-01-06 09:33:00", 97.0, 108.0, 100.0, 107.0),
        ]
    )
    ...
    result = run_case_from_frames(
        ...
        minute_df=minute_df,
        plan_factory=lambda **_kwargs: _trend_long_plan(),
        pre_market_cfg={"trend_initial_stop_grace_bars": 1},
        signal_cfg={},
    )
    ...
    assert result.trades[0].entry_time == "2025-01-06 09:31:00"
    assert result.trades[0].exit_reason == "tp2"
    assert result.diagnostics["phase3_initial_stop_grace_skips"] == 1
...
def test_run_case_from_frames_rebases_execution_levels_to_actual_trigger_price(monkeypatch):
    ...
    assert trade.meta["initial_stop_price"] == pytest.approx(101.0)
    assert trade.meta["initial_tp1_price"] == pytest.approx(109.0)
    assert trade.meta["initial_tp2_price"] == pytest.approx(112.0)
    assert trade.meta["entry_trigger_deviation"] == pytest.approx(3.0)
    assert trade.meta["entry_trigger_deviation_pct"] == pytest.approx(3.0 / 102.0)
    assert trade.meta["entry_rr"] == pytest.approx(1.0)
    assert trade.meta["entry_admission_rr"] == pytest.approx(1.75)
...
def test_run_case_from_frames_skips_trend_entry_when_confirmation_chases_too_far(monkeypatch):
    minute_df = _build_minute_df(
        [
            ("2025-01-06 09:31:00", 107.0, 107.5, 106.5, 107.0),
            ("2025-01-06 09:32:00", 107.0, 112.5, 106.5, 112.0),
        ]
    )

    monkeypatch.setattr(
        "backtest.phase23._safe_generate_signals",
        lambda df, direction, cfg: [{"type": "加多", "entry_family": "trend", "entry_signal_type": "TrendBreak"}],
    )
    ...
    result = run_case_from_frames(
        ...
        minute_df=minute_df,
        plan_factory=lambda **_kwargs: _trend_long_plan(),
        pre_market_cfg={"trend_max_entry_adverse_deviation_r": 1.0},
        signal_cfg={},
    )

    assert result.trades == []
    assert result.diagnostics["phase3_entry_adverse_deviation_rejects"] == 2


def test_run_case_from_frames_allows_legacy_trend_entry_chase_by_default(monkeypatch):
    minute_df = _build_minute_df(
        [
            ("2025-01-06 09:31:00", 107.0, 107.5, 106.5, 107.0),
            ("2025-01-06 09:32:00", 107.0, 112.5, 106.5, 112.0),
    ...
    assert len(result.trades) == 1
    assert result.diagnostics["phase3_entry_adverse_deviation_rejects"] == 0
