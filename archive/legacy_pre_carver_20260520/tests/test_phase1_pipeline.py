...
def test_build_candidate_exposes_trend_story_budget_from_medium_term_structure(monkeypatch) -> None:
    dates = pd.date_range("2025-01-01", periods=160, freq="D")
    closes = [100.0 + i * 0.45 for i in range(150)]
    closes.extend([167.0, 166.2, 165.8, 166.5, 167.4, 168.1, 169.0, 170.0, 170.8, 171.5])
    df = pd.DataFrame(
        {
            "date": dates,
            "open": closes,
            ...
            "close": closes,
            "volume": [1000.0 + i * 2.0 for i in range(160)],
            "oi": [5000.0 + i * 20.0 for i in range(160)],
        }
    )
    ...
    assert candidate["trend_story_quality_status"] == "candidate_v1"
    assert candidate["trend_story_bucket"] == "high"
    assert candidate["trend_structure_bucket"] == "healthy_continuation"
    assert candidate["trend_story_budget_margin_pct"] == 0.30
...
def test_apply_portfolio_position_sizing_logs_zero_lot_downgrade(monkeypatch) -> None:
    ...
    assert rows[0]["sizing_limited_by"] == "stop_risk"
    assert rows[0]["sizing_zero_lot_reason"] == "single_trade_stop_risk_below_one_lot"
...
def test_apply_portfolio_position_sizing_can_use_phase1_story_budget(monkeypatch) -> None:
    seen_logs: list[str] = []
    monkeypatch.setattr(daily_workflow.log, "info", lambda msg, *args: seen_logs.append(msg % args if args else msg))
    ...
    assert rows[0]["position_budget_source"] == "phase1_story"
    assert rows[0]["score_margin_budget"] == 300_000.0
    assert rows[0]["score_lots"] == 30
    assert rows[1]["score_margin_budget"] == 30_000.0
    assert rows[1]["score_lots"] == 3
    assert rows[0]["position_weight"] == 0.30
    assert rows[1]["position_weight"] == 0.03
    assert any("Phase1故事预算" in line and "高质量趋势" in line for line in seen_logs)
